...
class VentasView(ft.Column):
    ...
    def handle_file(self, e: ft.FilePickerResultEvent):
        try:
            ...
            file_obj = e.files[0]
            print(f"📁 Archivo seleccionado: {file_obj.name}")
            print(f"📁 Path: {file_obj.path}")
            print(f"📁 Size: {file_obj.size}")

            # En WebView, intentar leer el archivo desde la ruta
            file_path = file_obj.path
            
            if file_path and os.path.exists(file_path):
                print(f"✅ Ruta disponible, leyendo desde: {file_path}")
                ok, msg = import_csv_to_db(file_path)
            else:
                # Si no hay ruta, intentar desde el nombre en directorios comunes
                print("⚠️ Path no disponible, buscando en directorios comunes...")
                possible_paths = [
                    os.path.expanduser(f"~/Downloads/{file_obj.name}"),
                    os.path.expanduser(f"~/Desktop/{file_obj.name}"),
                    f"/tmp/{file_obj.name}",
                ]
                ...
                for p in possible_paths:
                    if os.path.exists(p):
                        ...
                        ok, msg = import_csv_to_db(p)
                        found = True
                        break
                
                if not found:
                    # Última alternativa: usar import_csv_from_content si está disponible
                    print("⚠️ Archivo no encontrado en directorios comunes")
                    print(f"❌ No se puede localizar el archivo: {file_obj.name}")
                    self.output.value = f"❌ Error: no se pudo acceder al archivo"
                    if self.page:
                        self.page.update()
                    return
            ...
            print(f"❌ Error en handle_file: {ex}")
            import traceback
            traceback.print_exc()
            self.output.value = f"❌ Error al procesar archivo: {ex}"
            if self.page:
                self.page.update()
