...
class DashboardView(ft.Column):
    ...
    def _handle_dropped_file(self, file_path):
        ...
        # En webview, puede venir como lista o como string
        if isinstance(file_path, list):
            file_path = file_path[0] if file_path else None
            print(f"📁 Convertido de lista a: {file_path}")
        
        if not file_path:
            print(f"❌ Archivo no válido")
            return
            
        file_path = str(file_path).strip()
        
        if not os.path.exists(file_path):
            print(f"❌ Ruta no existe: {file_path}")
        ...
        if not (file_path.endswith('.csv') or file_path.endswith('.xlsx')):
            print(f"❌ Solo se aceptan archivos CSV o XLSX. Recibido: {file_path}")
            return
